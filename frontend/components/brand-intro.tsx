      ...
      <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20">
        <Reveal className="order-2 lg:order-1">
          <p className="mb-6 flex items-center gap-3 text-[12px] font-medium uppercase tracking-[0.24em] text-accent">
            ...
            {t('brand.eyebrow', 'Our Philosophy')}
          </p>
          <h2 className="max-w-[18ch] text-balance font-serif text-4xl leading-[1.1] text-foreground sm:text-5xl">
            {t('brand.title', 'Every journey begins with a story')}
          </h2>
          <div className="mt-8 space-y-6 text-lg leading-relaxed text-muted-foreground">
            <p>
              {t('brand.p1', 'For thousands of years, Ethiopia has welcomed explorers, traders, pilgrims, artists, and dreamers. It is a land of rock-hewn churches, highland kingdoms, and the birthplace of coffee.')}
            </p>
            <p>
              {t('brand.p2', 'EthioAfro exists to help you experience that legacy through journeys crafted with care, knowledge, and authenticity — not packages, but experiences designed around the traveler you are.')}
            </p>
          </div>
            ...
            {[
              ...
              <p key={bold} className="text-sm leading-relaxed">
                <span className="block font-serif text-xl text-foreground">
                  {t(`brand.principle.${index}.bold`, bold)}
                </span>
                <span className="text-muted-foreground">{t(`brand.principle.${index}.rest`, rest)}</span>
              </p>
            ))}
          </div>
        </Reveal>
